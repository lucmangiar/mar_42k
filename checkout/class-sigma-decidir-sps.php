<?php
/**
 * Sigma Event's Implementation of DECIDIR SPS.
 *
 * Outputs a form with standard variables to POST to Decidir.
 * Creates a endpoint to process POSTs from Decidir with post-transaction information.
 * Updates the database according to the payment status received from the Decidir.
 * Logs all the POSTs to Decidir url. i.e. URL submitted for Decidir.
...
if ( !class_exists('Sigma_Decidir_SPS') ) :
/**
 * Sigma Decidir SPS Payment Processor
 *
 ...
 * @subpackage  PaymentProcessing
 */
class Sigma_Decidir_SPS extends Sigma_Payment_Processor
{
    /**
     ...
     * @var string
     */
    protected $data_log  = 'decidir_payments_data.log';
    protected $error_log = 'decidir_payments_error.log';
    ...
    function __construct( $registration_table, $payment_table ){
        ...
        add_action('init', array($this, 'decidir_rewrite'));
    ...
    function get_form( $operation_number, $amount, $means_of_payment, $submit ){
        ...
        if( 'decidir_visa' == $means_of_payment ):
            $means_of_payment = 1;
        elseif( 'decidir_amex' == $means_of_payment ):
            $means_of_payment = 6;
        elseif( 'decidir_mastercard' == $means_of_payment ):
            $means_of_payment = 15;
        endif;

        // Premium Event?
        if($amount > 0):
            $form = '<form action="' . $this->decidir_url . '" id="se-decidir-form" method="post" >';
            // Shop Number.
            ...
            // Medio De Pago.
            $form .= '<input type="hidden" name="MEDIODEPAGO" value="' . $means_of_payment . '" size=12 >';
            // Amount.
            $form .= '<input type="hidden" name="MONTO" value="' . $amount . '" size=12 maxlength=12 >';
            // Installments.
            $form .= '<input type="hidden" name="CUOTAS" value="' . '1' . '" size=12 >';
            // Dinamica URL.
            $form .= '<input type="hidden" name="URLDINAMICA" value="' . get_home_url() . '/' . $this->decidir_endpoint . '" >';

            $form .= $submit . '</form>';

        // Free Event?
        else:
            $form  = $this->get_free_event_form( $operation_number, $this->decidir_endpoint );
        endif;
     ...
     */
    function decidir_rewrite(){
        $decidir_regex = $this->decidir_endpoint . '/?$';
        add_rewrite_rule($decidir_regex,
        'index.php?' . $this->decidir_endpoint . '=sigma', 'top');
        add_rewrite_tag('%' . $this->decidir_endpoint . '%', '([^&]+)');
    }
